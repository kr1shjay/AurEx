...
//import action
import { editPair } from '../../actions/p2pAction'
...
class PairAddModal extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (!isEmpty(record)) {
            this.setState({
                formValue: {
                    'pairId': record._id,
                    "firstCoinId": record.firstCoinId,
                    "secondCoinId": record.secondCoinId,
                    "feePct": record.feePct,
                    "markPrice": record.markPrice,
                    "fetchMarkPrice": record.fetchMarkPrice,
    ...
    handleSubmit = async e => {
        ...
        try {
            ...
            let { status, loading, error, message } = await editPair(reqData);
            this.setState({ 'loader': loading })
            if (status == 'success') {
                toastAlert('success', message, 'editP2p');
                this.handleClose()
                ...
                toastAlert('error', message, 'editP2p');
            }
        }
        catch (err) { }
    }
            ...
            <div>
                ...
                >
                    <Modal.Header closeButton>
                        <h4 className="modal-title">Update P2P Pair</h4>
                    </Modal.Header>
                    <Modal.Body>
                        ...
                        <form noValidate id="add-spot">
                            ...
                            {
                                ['local'].includes(fetchMarkPrice) && <div className="row mt-2">
                                    <div className="col-md-3">
