...
function convertabcdHistoryToUDFFormat(data, callBy) {
	...
	function formatDate(dateInput) {
		...
		// var finaltimex = datex.getUTCHours() + ":" + datex.getUTCMinutes() + ":" + datex.getUTCSeconds() + ":" + datex.getUTCMilliseconds();
		var finaltimex = datex.getUTCHours() + ":" + datex.getUTCMinutes() + ":" + datex.getUTCSeconds() + ":" + datex.getUTCMilliseconds();
...
RequestProcessor.prototype._sendSymbolInfo = function (symbolName, response, tradeType) {
...
RequestProcessor.prototype._sendSymbolHistory = async function (symbol, startDateTimestamp, endDateTimestamp, resolution, response, tradeType) {
	...
	if (tradeChart && tradeChart.chartData && tradeChart.chartData.length > 0) {
		var data = convertabcdHistoryToUDFFormat(JSON.stringify(tradeChart.chartData), tradeChart.callBy);
		if (data === null) {
...
RequestProcessor.prototype.processRequest = function (action, query, response, tradeType) {
	...
	if (action === "/config") {
		...
		this._sendSymbolHistory(query["symbol"], query["from"], query["to"], query["resolution"].toLowerCase(), response, tradeType);
