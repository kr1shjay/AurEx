...
export const getDetail = async (req, res) => {
    try {
        ...
        if (!isEmpty(reqQuery.userId)) {
            findOrder['userId'] = { "$ne": ObjectId(reqQuery.userId) }
        }
...
export const checkPost = async (req, res, next) => {
    try {
        ...
        let usrSetting = await UserSetting.findOne({ 'userId': ObjectId(postData.userId) });
...
export const orderPlace = async (req, res) => {
    try {
        ...
        console.log("-----reqBody.side ", reqBody.side)
        if (reqBody.side == 'sell') {
...
export const cancelOrder = async (req, res) => {
    try {
        ...
        if (updateDoc.side == 'sell') {
            ...
            orderValue = toFixed(orderValue, 8)
            console.log("--------cancelOrder-------", orderValue)
            await Wallet.updateOne({
                "_id": updateDoc.sellUserId,
                'assets._id': updateDoc.firstCoinId
            }, {
...
export const disputeOrder = async (req, res) => {
    try {
        let reqParam = req.params;
        let orderData = await P2pOrder.findOne({ "_id": reqParam.orderId }).populate({ 'path': "postId", 'select': "status" });
        console.log("🚀 ~ file: p2p.controller.js ~ line 1810 ~ disputeOrder ~ orderData", orderData)
    ...
    try {
        ...
        if (!isEmpty(reqQuery.side)) {
            if (reqQuery.side == 'all') {
                filter['$or'] = [{ "buyUserId": ObjectId(req.user.id) }, { "sellUserId": ObjectId(req.user.id) }]
            ...
            for (let orderData of orderList) {
                if (orderData && orderData.postId) {
                    if (orderData.postId.side == 'sell') {
                        ...
                        balance = toFixed(balance, 8)
                        console.log("-------balance---p2pOrderCrossTime", balance)
                        console.log("-------orderData.receiveValue---p2pOrderCrossTime", orderData.receiveValue)
                        await Wallet.updateOne({
        ...
        if (postList && postList.length > 0) {
            for (let postData of postList) {
                if (postData.side == 'sell') {
                    ...
                    balance = toFixed(balance, 8)
                    console.log("-------postData.quantity-----p2pPostCrossDate", postData.quantity)
                    console.log("-------postData.filledQuantity-----p2pPostCrossDate", postData.filledQuantity)
                    console.log("-------remainingQty-----p2pPostCrossDate", remainingQty)
                    console.log("--------balance-----p2pPostCrossDate", balance)
                    await Wallet.updateOne({
                        "_id": postData.userId,
    ...
    try {
        ...
        if (postData.status == 'open') {
            if (postData.side == 'sell') {
                ...
                console.log("-------cancel----post", orderValue)
                usrAsset.p2pBal = usrAsset.p2pBal + orderValue;
                await usrWallet.save()
            }
...
export const cancelMyad = async (req, res) => {
    try {
        ...
        if (Tradedata.BuyorSell == "Sell") {
            ...
            await Assets.findOneAndUpdate({ _id: ObjectId(assets._id) }, {
                $set: {
...
export const disputeTrade = async (req, res) => {
    try {
        ...
        var tradetable = await P2PTradeTable.findOne({ _id: ObjectId(orderbookdata.trade_id) });
        var Sender_userId = ObjectId(reqBody.userid);
        var Receiver_userId = orderbookdata.from_userId == reqBody.userid ? ObjectId(orderbookdata.to_userId) : ObjectId(orderbookdata.from_userId);
        ...
        chatsave.save().then(async (saved) => {
            if (saved) {
                await P2POrderbook.findOneAndUpdate({ _id: ObjectId(orderbookdata._id) }, { $set: { status: 3 } });
                ...
                chatsave.save().then(async (saved) => {
                    if (saved) {
                        ...
                        if (tradetable.BuyorSell == 'Buy') {
                            const assets = await Assets.findOne({ userId: ObjectId(orderbookdata.from_userId), currencySymbol: crypto_currency });
                            var current_cryptobalance = assets.p2pWallet;
                            var new_cryptobalance = current_cryptobalance + refund_amount;
                            var holdingbalance = assets.p2pholdingbalance - refund_amount;
                            await Assets.findOneAndUpdate({ _id: ObjectId(assets._id) }, {
                        ...
                        await P2PTradeTable.findOneAndUpdate({ _id: ObjectId(tradetable._id) }, {
                            $set: {
                        ...
                        // Update trade Status
                        await P2POrderbook.findOneAndUpdate({ _id: ObjectId(orderbookdata._id) }, {
                            $set: {
                                status: 5,
                            },
                        });
            ...
            if (!(check2Fa && check2Fa.ditema == 0)) {
                ...
                chatsave.save().then(async (saved) => {
                    if (saved) {
                        ...
                        var crypto_currency = tradetable.firstCurrency;
                        var assets = await Assets.findOne({ userId: ObjectId(orderbookdata.to_userId), currencySymbol: crypto_currency });
                        var current_cryptobalance = assets.p2pWallet;
                        var crypto_amount = orderbookdata.crypto_amount;
                        var new_cryptobalance = current_cryptobalance + crypto_amount;
                        await Assets.findOneAndUpdate({ _id: ObjectId(assets._id) }, { $set: { p2pWallet: new_cryptobalance } });
                        ...
                        Assets.findOneAndUpdate({ userId: ObjectId(orderbookdata.from_userId), currencySymbol: orderbookdata.firstCurrency }, { $inc: { p2pholdingbalance: -holding_balance } }, { new: true }, function (err, dd) {
                        });

                        // Update trade Status
                        await P2POrderbook.findOneAndUpdate({ _id: ObjectId(orderbookdata._id) }, { $set: { status: 4 } });
                        await P2PTradeTable.findByIdAndUpdate({ _id: ObjectId(TradeId) }, { $set: { status: 0 } });

                        if (tradetable.maxlimit == 0) {
                            await P2PTradeTable.findByIdAndUpdate({ _id: ObjectId(TradeId) }, { $set: { status: 1 } });
                        }
                        ...
                        let wallet = await Assets.findOne({ userId: ObjectId(userId), currencySymbol: crypto_currency }).lean();
...
export const confirmPay = async (req, res) => {
    try {
        ...
        chatsave.save().then(async (saved) => {
            if (saved) {
                await P2POrderbook.findOneAndUpdate({ _id: ObjectId(orderbookdata._id) }, { $set: { status: 1 } });
    ...
    try {
        ...
        chatsave.save().then(async (saved) => {
            if (saved) {
                ...
                // Update trade Status
                await P2POrderbook.findOneAndUpdate({ _id: ObjectId(orderbookdata._id) }, { $set: { status: 2 } });
                // End
                let checkUser = await User.findOne({ _id: orderbookdata.from_userId });
                var pair = orderbookdata.firstCurrency + "-" + orderbookdata.secondCurrency;
                let content = {
        ...
        if (Tradedata) {
            try {
                ...
                if (Tradedata.BuyorSell == "Buy") {
                    var assets = await Assets.findOne({ userId: ObjectId(reqBody.userid), currencySymbol: crypto_currency });
                    var current_cryptobalance = assets.p2pWallet;
                ...
                Orderbook.save().then(async (saved) => {
                    if (saved) {
                        ...
                        await P2PTradeTable.findOneAndUpdate({ _id: ObjectId(Tradedata._id) }, {
                            $set: {
                        ...
                        var crypto_currency = Tradedata.firstCurrency;
                        if (Tradedata.BuyorSell == "Buy") {
                            const assets = await Assets.findOne({ userId: ObjectId(reqBody.userid), currencySymbol: crypto_currency });
                            var current_cryptobalance = assets.p2pWallet;
                            var crypto_amount = reqBody.crypto_amount;
                            var fee_amount = crypto_amount * (Tradedata.fee_percentage / 100);
                        ...
                        let wallet = await Assets.findOne({ userId: ObjectId(reqBody.userid), currencySymbol: crypto_currency }).lean();
