...
export const sentOtp = async (req, res) => {
    try {
        ...
        console.log("🚀 ~ file: user.controller.js ~ line 139 ~ sentOtp ~ err", err)
        return res.status(500).json({ "success": false, 'message': "SOMETHING_WRONG" })
    }
}
...
export const createNewUser = async (req, res) => {
    try {
        ...
        if (reqBody.formType == 'email') {
            ...
            if (checkDoc) {
                return res.status(400).json({ "success": false, 'erros': { 'phoneNo': "Phone number already exist" } })
            }
            let to = `+${reqBody.phoneCode}${reqBody.phoneNo}`;
        ...
        if (!isEmpty(reqBody.referenceCode)) {
            ...
            if (!referenceDoc) {
                return res.status(500).json({ "success": false, 'errors': { 'referenceCode': "Invalid referal code" } })
            }
    ...
    try {
        ...
        if (currencyData && currencyData.length > 0) {
            for (let currency of currencyData) {
                console.log("-------currency.type", currency.type, req.user.id)
...
export const userLogin = async (req, res) => {
    try {
        ...
        if (reqBody.formType == 'email') {
            ...
            if (!checkUser) {
                return res.status(404).json({ 'success': false, 'errors': { 'phoneNo': "Mobile number not found" } });
            }
        ...
        if (checkUser.hash == "" && checkUser.hash == "") {
            ...
            return res.status(400).json({ 'success': false, 'message': "Your Password is Old Please Reset Your Password", "authToken": encryptToken });
        }
        ...
        return res.status(200).json({ 'success': true, 'status': "SUCCESS", 'message': "Login successfully", token, result, userSetting })
...
export const checkForgotPassword = async (req, res) => {
    try {
        ...
        if (reqBody.type == 'mobile') {
            ...
            if (smsStatus === true) {
                return res.status(200).json({ "success": true, message: 'success', result: encryptToken })
            }
        ...
        if (reqBody.type == 'email') {
            ...
            return res.status(200).json({ 'success': true, "message": "Confirmation Link Sent to Your Mail" })
        }
...
export const changeNewPhone = async (req, res) => {
    try {
        ...
        if (checkUser) {
            ...
            if (checkUser._id.toString() == req.user.id) {
                return res.status(400).json({ "success": false, 'errors': { 'newPhoneNo': "Does matched your previous mobile number" } })
            }
        }
        ...
        if (!smsStatus) {
            return res.status(500).json({ "success": false, 'message': "Please enter your valid mobile number" })
        }
        ...
        console.log("🚀 ~ file: user.controller.js ~ line 1232 ~ changeNewPhone ~ err", err)
        return res.status(500).json({ "success": false, 'message': "SOMETHING_WRONG" })
    }
}
