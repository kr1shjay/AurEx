...
export const checkMobile = async (req, res, next) => {
  try {
    ...
    if (reqBody.type == "register" && checkDoc) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Phone number not exist" });
    }
  ...
  try {
    ...
    if (reqBody.type == "login") {
      ...
      if (isEmpty(checkDoc) && reqBody.type == "forgot") {
        return res.status(400).json({
          success: false,
          errors: { phone: "Phone number not exists" },
        });
      }
    ...
    if (isEmpty(checkDoc) && reqBody.type == "forgot") {
      return res
        .status(400)
        .json({ success: false, errors: { phone: "Phone number not exists" } });
    }
    let to = `+${reqBody.phoneCode}${reqBody.phoneNo}`;
    let { smsStatus } = await smsHelper.sentOtp(to);
    // console.log(smsStatus, 'smsStatussmsStatus')
    if (!smsStatus) {
      return res
        .status(400)
        .json({ success: false, errors: { phoneNo: "Invalid mobile number" } });
    }
    ...
    console.log(
      "🚀 ~ file: user.controller.js ~ line 139 ~ sentOtp ~ err",
      err
    );
    return res.status(500).json({ success: false, message: "SOMETHING_WRONG" });
  }
...
export const createNewUser = async (req, res) => {
  try {
    ...
    if (reqBody.formType == "email") {
      ...
      let checkDoc = await User.findOne({
        phonceCode: reqBody.phoneCode,
        phoneNo: reqBody.phoneNo,
      });
      if (checkDoc) {
        return res.status(400).json({
          success: false,
          erros: { phoneNo: "Phone number already exist" },
        });
      }
      let to = `+${reqBody.phoneCode}${reqBody.phoneNo}`;
    ...
    if (!isEmpty(reqBody.referenceCode)) {
      ...
      if (!referenceDoc) {
        return res.status(500).json({
          success: false,
          errors: { referenceCode: "Invalid referal code" },
        });
      }
    ...
    let message =
      "Your account has been registered. Please check your email and enable your account.";
  ...
  try {
    ...
    if (currencyData && currencyData.length > 0) {
      for (let currency of currencyData) {
        console.log("-------currency.type", currency.coin);
        if (currency.type == "crypto") {
          // console.log('cryptoooooooooooooooooooooo')
        ...
        if (currency.type == "token") {
          ...
          if (currency.tokenType == "erc20") {
            // console.log(currency.tokenType, "erc20erc20");
            ethCtrl.ERC20_Deposit(userData.userId, currency.coin);
          }
        }
      }
    }
...
export const userLogin = async (req, res) => {
  try {
    ...
    if (reqBody.formType == "email") {
      ...
      if (!checkUser) {
        return res.status(404).json({
          success: false,
          errors: { phoneNo: "Mobile number not found" },
        });
      }
    ...
    if (checkUser.hash == "" && checkUser.hash == "") {
      ...
      return res.status(400).json({
        success: false,
        message: "Your Password is Old Please Reset Your Password",
    ...
    if (checkUser.google2Fa && !isEmpty(checkUser.google2Fa.secret)) {
      if (isEmpty(reqBody.twoFACode)) {
        return res.status(200).json({
          success: true,
          status: "TWO_FA",
          message: "Please Enter Your 2 FA Code",
        });
      } else {
...
export const update2faCode = async (req, res) => {
  try {
    ...
    // console.log(check2Fa, 'check2Facheck2Fa')
    if (check2Fa && check2Fa.delta == 0) {
...
export const generateTwoFa = (userData) => {
  ...
  if (userData && userData.google2Fa.secret != "") {
    ...
    let newSecret = node2fa.generateSecret({
...
export const editNotif = async (req, res) => {
  try {
    ...
    if (reqBody.name == "twoFA") {
      message = reqBody.checked
        ? "2FA Alert Enabled Successfully"
        : "2FA Alert Disabled Successfully";
    } else if (reqBody.name == "passwordChange") {
      message = reqBody.checked
        ? "Login Password Alert Enabled Successfully"
        : "Login Password Alert Disabled Successfully";
    }
  ...
  try {
    ...
    if (reqBody.type == "mobile") {
      ...
      if (smsStatus === true) {
        return res
          .status(200)
          .json({ success: true, message: "success", result: encryptToken });
      }
    }
    if (reqBody.type == "email") {
      ...
      if (!userData) {
        return res
          .status(400)
          .json({ success: false, errors: { email: "EMAIL_NOT_EXISTS" } });
      }
      ...
      return res.status(200).json({
        success: true,
        message: "Confirmation Link Sent to Your Mail",
      });
    }
  } catch (err) {
...
export const changeNewPhone = async (req, res) => {
  try {
    ...
    let { smsStatus } = await smsHelper.sentSms(smsContent);
    if (!smsStatus) {
      return res.status(500).json({
        success: false,
    ...
    console.log(
      "🚀 ~ file: user.controller.js ~ line 1232 ~ changeNewPhone ~ err",
      err
    );
    return res.status(500).json({ success: false, message: "SOMETHING_WRONG" });
  }
...
export const verifyNewPhone = async (req, res) => {
  try {
    let reqBody = req.body,
      otpTime = new Date(new Date().getTime() - 600000); //2 min
    let userData = await User.findOne({ _id: req.user.id });
    ...
    if (
      userData.newPhone.phoneCode != reqBody.newPhoneCode ||
      userData.newPhone.phoneNo != reqBody.newPhoneNo
    ) {
      return res.status(400).json({
        success: false,
  ...
  try {
    ...
    if (!Details.email) {
      ...
      if (checkUser) {
        return res.status(400).json({
          success: false,
          errors: { newEmail: "Email already exists" },
        });
      }
    ...
    if (!isEmpty(FindUser)) {
      if (FindUser.status === "unverified") {
        ...
        if (!isEmpty(Update)) {
          return res
            .status(200)
            .json({ status: true, message: " verified user" });
        } else {
        ...
        if (!isEmpty(Update)) {
          return res
            .status(200)
            .json({ status: true, message: " unverified user" });
        } else {
          return res.status(400).json({ status: false, message: " Failed" });
        }
      } else {
    ...
    if (!isEmpty(checkUser)) {
      if (!isEmpty(checkUser.google2Fa.secret)) {
        ...
        let Data = await User.findOneAndUpdate(
          { _id: checkUser._id },
          { $set: { google2Fa: Disable } }
        );
        if (!isEmpty(Data)) {
          return res
            .status(200)
            .json({ status: true, message: "2FA Disabled" });
        } else {
          return res
            .status(400)
            .json({ status: false, message: "Disabl Failed" });
        }
      }
    }
  } catch (err) {
...
export const AddFavorite = async (req, res) => {
  try {
    let reqBody = req.body;
    let checkItem = await UserSetting.findOne({
      userId: req.user.id,
      favorite: reqBody.item,
    });
    if (!isEmpty(checkItem)) {
      ...
      if (!isEmpty(PushItem)) {
        return res
          .status(200)
          .json({ success: true, message: "Favorite Added" });
      } else {
...
export const getAllTrade = async (req, res) => {
  try {
    let spotComplete = await SpotTrade.find({
      userId: req.user.id,
      status: "completed",
    });
    ...
    //     { $match: { status: 'open' } }
    // ])
    let p2pOpenOrder = await P2pPost.find({
      userId: req.user.id,
      status: "open",
    });
