...
export const getHideZeroStatus = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, hideZeroStatus: hideZeroStatus });
  } catch (err) {
    return res.status(500).json({ success: false });
  }
};
...
export const getbalance = async (req, res) => {
  try {
    ...
    if (!balancedata) {
      return res
        .status(400)
        .json({ success: true, messages: "USER_NOT_FOUND" });
    }
    let assetDoc = balancedata.assets.find(
      (val) => val.coin == req.query.symbol
    );
    ...
    if (currencyList && currencyList.length > 0) {
      ...
      if (currencyList[0].token && currencyList[0].token.length > 0) {
        ...
        for (let tokenData of currencyList[0].token) {
          ...
          // console.log(tokenDoc, "tokenDoctokenDoc");
          walletData = await Wallet.findOneAndUpdate(
            {
              userId: userId,
...
export const checkUserKyc = async (req, res, next) => {
  try {
    ...
    if (
      ...
      userKyc.idProof.status == "pending" ||
      userKyc.addressProof.status == "pending"
    ) {
      return res.status(400).json({
        success: false,
        message: "Your KYC must be verified to proceed Deposit",
      });
    } else {
      return res
        .status(400)
...
export const withdrawFiatRequest = async (req, res) => {
  try {
    ...
    if (
      api_key !== null &&
      api_key !== undefined &&
      req.user.withdraw !== true
    ) {
      // console.log("orderreq444", req.user)
      return res.status(400).json({
        status: false,
        message: "You don't have permission to WithdrawFiatRequest",
      });
    } else {
      let reqBody = req.body;
      ...
      let finalAmount =
        reqBody.amount +
        precentConvetPrice(reqBody.amount, curData.withdrawFee);
      ...
      if (curData.minimumWithdraw > finalAmount) {
        return res.status(400).json({
          success: false,
          errors: { finalAmount: "WITHDRAW_TOO_LOW" },
        });
      }
...
export const fiatRequestVerify = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Successfully verifed withdraw request",
    });
  } catch (err) {
    return res.status(500).json({ success: false, message: "SOMETHING_WRONG" });
  }
...
export const withdrawCoinRequest = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        status: false,
        message: "You don't have permission to WithdrawCoinRequest",
      });
    } else {
      let reqBody = req.body;
      reqBody.amount = parseFloat(reqBody.amount);
      ...
      if (
        reqBody.coin != "XRP" &&
        usrAsset.address == reqBody.receiverAddress
      ) {
        return res.status(400).json({
          success: false,
          errors: { receiverAddress: "RECEIVER_ADDRESS_SHOULD_DIFFER" },
        });
      }
      ...
      if (reqBody.coin == "XRP" && usrAsset.destTag == reqBody.destTag) {
        return res.status(400).json({
          success: false,
          errors: { destTag: "RECEIVER_TAG_SHOULD_DIFFER" },
        });
      }
...
export const withdrawfee = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, messages: "success", result: finalAmount });
  } catch (err) {
...
export const walletTransfer = async (req, res) => {
  try {
    ...
    let newTrnx = new Transaction({
      ...
      status: "completed",
      paymentType:
        currencyDoc.type == "crypto" ? "coin_transfer" : "fiat_transfer",
    });
...
export const fundTransfer = async (req, res) => {
  try {
    ...
    if (!toUserData) {
      return res
        .status(500)
        .json({ success: false, errors: { toUserEmail: "Email Not Exit" } });
    }
    ...
    if (req.user.google2Fa && !isEmpty(req.user.google2Fa.secret)) {
      let verifyTwoFaCode = node2fa.verifyToken(
        req.user.google2Fa.secret,
        reqBody.twoFACode
      );
      if (!(verifyTwoFaCode && verifyTwoFaCode.delta == 0)) {
        return res
          .status(400)
          .json({ success: false, errors: { twoFACode: "Invalid Code" } });
      }
      ...
      let finalAmount =
        reqBody.amount + precentConvetPrice(reqBody.amount, curData.fundFee);
      if (usrAsset.spotBal < finalAmount) {
        return res.status(400).json({
          success: false,
          errors: { finalAmount: "INSUFFICIENT_BALANCE" },
      ...
      return res
        .status(200)
        .json({ success: true, message: "Amount transfer successfully" });
    } else {
      return res.status(500).json({
        success: false,
        errors: { twoFACode: "Please enable two factor authentication" },
      });
    }
  } catch (err) {
      ...
      if (exportData && exportData.length > 0) {
        for (let item of exportData) {
          ...
          arr.push(
            item.createdAt.toLocaleString(),
            item.userId,
            item.paymentType == "fiat_withdraw"
              ? item.bankDetail.accountNo
              : item.toAddress,
            item.coin,
            paymentType(item.paymentType),
            item.actualAmount,
...
export const WithdrawCancel = async (req, res) => {
  try {
    ...
    if (trxData && trxData.status != "pending") {
      return res.json({
        status: false,
        message: "Your Withdraw Already " + trxData.status,
      });
    }
  ...
  try {
    ...
    if (trxData.createdAt <= setTime) {
      if (trxData && trxData.status == "pending") {
        ...
        return res.json({
          status: false,
          message: "Your Withdraw Cancelled Because Time Expired!!!",
        });
      } else {
        return res.json({
          status: false,
...
export const fiatDepositApprove = async (req, res) => {
  try {
    ...
    if (trxData.amount < reqBody.amount) {
      return res.status(400).json({
        success: false,
        message: "Amount should be lower than depoist amount",
      });
    }
