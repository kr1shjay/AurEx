...
import { btcDepositTask } from '../../config/cron'
...
export const createAddress = async (data) => {
    try {
        let respData = await axios({
            'method': 'post',
            'timeout': 1000,
            'url': `${config.COIN_GATE_WAY.BTC.URL}/newAdress`,
            data
        });
...
btcDepositTask.start();
export const deposit = async () => {
    btcDepositTask.stop()
    console.log("BTC Deposit Cron");
    try {
        let currencyData = await Currency.findOne({ 'coin': "BTC" });
        if (!currencyData) {
            return false;
        }
        ...
        let respData = await axios({
            'method': "get",
            'url': `${config.COIN_GATE_WAY.BTC.URL}/transactionList`,
            'params': data,
        });
...
export const transfer = async ({ userAddress, amount }) => {
    try {
        ...
        let respData = await axios({
            'method': 'get',
            'url': `${config.COIN_GATE_WAY.BTC.URL}/transfer`,
            data
        });
        if (respData && respData.status == 200 && respData.data.status == 'SUCCESS') {
            const { result } = respData.data
