...
export const createAddress = async () => {
    try {
        let respData = await axios({
            'method': 'get',
            timeout: 1000,
            'url': `${config.COIN_GATE_WAY.ETC.URL}/getnewaddress`,
        });
...
export const amountMoveToAdmin = async (data) => {
    try {
        let respData = await axios({
            'method': 'post',
            'url': `${config.COIN_GATE_WAY.ETC.URL}/eth-move-to-admin`,
            data
        });
...
export const tokenMoveToAdmin = async (data) => {
    try {
        let respData = await axios({
            'method': 'post',
            'url': `${config.COIN_GATE_WAY.ETC.URL}/erc20-token-move-to-admin`,
            data
        });
...
export const getLatestBlock = async () => {
    try {
        let respData = await axios({
            'method': 'get',
            'url': `${config.COIN_GATE_WAY.ETC.URL}/getLatestBlock`,
        })
...
export const updateDeposit = async (transactionsData, currencyData) => {
    try {
        ...
        if (userAssetData) {
            let respData = await axios({
                'method': 'post',
                'url': `${config.COIN_GATE_WAY.ETC.URL}/getBalance`,
                data: {
            ...
            if (respData && respData.data) {
                ...
                if (balance > 0) {
                    ...
                    let fromWeiRespData = await axios({
                        'method': 'post',
                        'url': `${config.COIN_GATE_WAY.ETC.URL}/fromWei`,
                        data: {
...
//         let respData = await axios({
//             'method': 'post',
//             'url': `${config.COIN_GATE_WAY.ETC.URL}/amountMoveToAdmin`,
//             'data': {
//                 userAddress,
//                 userPrivateKey,
...
export const amountMoveToUser = async (data) => {
    try {
        ...
        let respData = await axios({
            'method': 'post',
            'url': `${config.COIN_GATE_WAY.ETC.URL}/eth-move-to-user`,
            data
        });
...
export const tokenMoveToUser = async (data) => {
    try {
        let respData = await axios({
            'method': 'post',
            'url': `${config.COIN_GATE_WAY.ETC.URL}/erc20-token-move-to-user`,
            data
        });
        if (respData && respData.status == 200) {
            return {
