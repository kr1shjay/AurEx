...
import SpotTrade from '../../models/spottradeTable';
...
export const getbalance = async (req, res) => {
    try {
        ...
        let buyOrders = await SpotTrade.find({
            userId: ObjectId(req.user.id),
            "status": { "$in": ['open', 'pending'] },
            "buyorsell": 'buy',
            'secondCurrency': req.body.symbol
        })
        let buyinvest = 0
        console.log("buyOrders", buyOrders)

        if (buyOrders.length > 0) {
            buyOrders?.map((val, index) => {
                buyinvest = buyinvest + val.orderValue
            })
        }


        let sellOrders = await SpotTrade.find({
            userId: ObjectId(req.user.id),
            "status": { "$in": ['open', 'pending'] },
            "buyorsell": 'sell',
            'firstCurrency': req.body.symbol
        })

        console.log("sellOrders", sellOrders)
        let sellinvest = 0
            if (sellOrders.length > 0) {
                sellOrders?.map((val, index) => {
                    sellinvest = sellinvest + val.orderValue
                })
            }
        

        let frozenCoin = buyinvest + sellinvest
        console.log("frozenCoin", buyinvest, sellinvest, frozenCoin)


        let result = {
            "coin": assetDoc.coin,
            "coinbal": assetDoc.spotBal,
            "Frozen coin": frozenCoin
        }
