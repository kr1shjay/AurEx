...
const StakingFaq = require('../../models/stakingfaq');
...
router.get('/get_staking_btn', (req, res) => {
   
    var findData = {};
    // findData.identifier = "TRANSPARENT";
    findData['$or'] = [];
    findData['$or'].push({'identifier':"Main Staking Button"});
    findData['$or'].push({'identifier':"Main Staking Content"});
    findData['$or'].push({'identifier':"Staking Table Button 1"});
    findData['$or'].push({'identifier':"Staking Table Button 2"});
    Cms.find(findData,{_id:0,__v:0}).then(response => {
        if (response) {
...
router.get('/get_staking_faq', (req, res) => {
   
    StakingFaq.find({}).then(response => {
        if (response) {
            return res.status(200).send(response);
        }
    });
