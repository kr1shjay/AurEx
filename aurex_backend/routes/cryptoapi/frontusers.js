...
const  {sendEmail} = require("../../lib/emailGateway") ;
...
router.get("/get-latest-stakingcount", async (req, res) => {
  console.log("allowed111")
  var cond = { createdDate: { $gt: new Date(Date.now() - 86400000) } }
  let getcount = await stakingOrder.find({ createdDate: { $gt: new Date(Date.now() - 86400000) } }).count();
  let totalinv = await stakingOrder.find({ createdDate: { $gt: new Date(Date.now() - 86400000) } })
  console.log("totalinv", totalinv)
  var length = 0;
  var data = totalinv;
  var res1 = 0;
  for (var i = 0; i < totalinv.length; i++) {
    // if(result[i].type == 'join'){
    length = length + 1
    // }

    res1 = res1 + parseFloat(totalinv[i].amount);
  }
  console.log("amount", res1)

  // var result1={res1,length,data}


  res.send({ 'status': 200, 'result': (getcount && getcount > 0) ? getcount : 0, 'totalinv': res1 })
});
          ...
          function (done) {
            ...
            currency
              ...
              .then((currencytokendetails) => {
                console.log("currencytokendetails", currencytokendetails)
    ...
    if (user.authenticate(req.body.password)) {
      ...
      if (user.active == "Activated") {
        ...
        // }
         ...
         console.log('sitedetails',sidedetails);
              ...
              Emailtemplates.findOne(jsonfilter, { _id: 0 }, function (
                ...
                if (templates.content) {
                  templateData = templates;
                  templateData.content = templateData.content.replace(
                    /##TEMPLATEINFO_COMPANY_NAME##/g,
                    sitedetails.companyName
                  );
                  templateData.content = templateData.content.replace(
                    /##TEMPLATEINFO_SITE_NAME####/g,
                    sitedetails.siteName
                  );
                  templateData.content = templateData.content.replace(
                    /##TEMPLATEINFO_SUPPORT_MAIL##/g,
                    sitedetails.supportMail
                  );
                  templateData.content = templateData.content.replace(
                    /##TEMPLATEINFO_ADDRESS##/g,
                    sitedetails.address
                  );email
                  templateData.content = templateData.content.replace(
                    /##TEMPLATEINFO_CONTACT_NO##/g,
                    sitedetails.contactNo
                  );
                  ...
                  // };
                  var to= email;
                  var emailcontent = templateData.content;
                  var to = "mangaleshwaran@britisheducationonline.org"
...
async function emailCheck(){
  SiteSetting.findOne({companyName:'Wealwin'}).exec(function(err,sitedetails){
    ...
    Emailtemplates.findOne(jsonfilter, { _id: 0 }, function (
      ...
      if (templates.content) {
        ...
        templateData.content = templateData.content.replace(
          /##TEMPLATEINFO_FACEBOOK_URL##/g,
          sitedetails.facebookUrl
        );
        templateData.content = templateData.content.replace(
          /##TEMPLATEINFO_TWITTER_URL##/g,
          sitedetails.twitterUrl
        );  templateData.content = templateData.content.replace(
          /##TEMPLATEINFO_LINKEDIN_URL##/g,
          sitedetails.linkedinUrl
        );  templateData.content = templateData.content.replace(
          /##TEMPLATEINFO_TWITTER_ICON##/g,        
          sitedetails.twitterIcon
        );  templateData.content = templateData.content.replace(
          /##TEMPLATEINFO_FACEBOOK_ICON##/g,
          sitedetails.facebookIcon
        );  templateData.content = templateData.content.replace(
          /##TEMPLATEINFO_LINKEDIN_ICON####/g,
          sitedetails.linkedinIcon
        );
